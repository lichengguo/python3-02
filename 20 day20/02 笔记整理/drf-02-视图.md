...
创建一个单独的子应用 myview, 方便我们接下来的学习

```python
cd demo/apps
python ../../manage.py startapp myview
```
...
`request.data` 返回解析之后http请求体数据。类似于Django中标准的`request.POST`和 `request.FILES`属性，但提供如下特性：
...
class Student4GenericAPIView(RetrieveUpdateAPIView):
	queryset = Student.objects.all()
	serializer_class = StudentSerializer
```
...
class StudentModelViewSet(ModelViewSet):
    ...
    def get_new_5(self,request):
        """获取最近添加的5个学生信息"""
        # 操作数据库
        print(self.action) # 获取本次请求的视图类方法名: 会打印 get_new_5
