...
class Basic:
    ...
    def write(self, name, file):  # 创建课程列表或学生列表
        with open('db_info/%s' % file, encoding='utf-8', mode='a+') as f:
            f.write('%s\n' % name)
            return True

    def write_dic(self,stu_name, stu_dict):  # 把学生信息字典写入到文件
        print()
...
class Student(Basic):
    ...
    def xuanke(self):  # 选择课程
        kecheng_list = self.read('kecheng_list')
        print('可选课程:%s' % kecheng_list)
        choice = input('\n请输入你想选择的课程>>>:').strip()
    ...
    def see_all_xuanke(self):  # 查看所选课程
        stu_dic = self.read_dic(user_name)
        print('%s 所选的课程有 %s' % (user_name, stu_dic['course']))
    ...
    def see_stu_xuanke(self):  # 查看所有学生选课情况
        ...
        if student_lis:
            for stu_name in student_lis:
                ...
                print('%s 的选课情况: %s' % (stu_name,stu_dic['course']))
        else:
            print('还没有学生')
